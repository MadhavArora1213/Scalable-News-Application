...
<div class="admin-content">
    <div class="admin-container">
        <form action="<?= SITE_URL ?>/admin/articles/<?= $article['id'] ?>/update" method="POST" enctype="multipart/form-data">
                ...
                <div class="main-content">
                    <div class="admin-card">
                        <div class="form-group">
                            ...
                            <input type="text" id="title" name="title" class="form-control form-control-lg" style="font-size: 32px; font-weight: 900; background: transparent; border: none; border-bottom: 2px solid var(--admin-border); border-radius: 0; padding-left: 0; padding-right: 0;" value="<?= htmlspecialchars($article['title']) ?>" minlength="20" title="Headline must be at least 20 characters long" required>
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 8px;">
                                ...
                                <small id="title-counter" style="color: var(--admin-text-muted); font-size: 0.75rem; font-weight: 600;"><?= strlen($article['title']) ?> characters</small>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div id="editor" style="min-height: 500px; border-radius: 12px; overflow: hidden;"></div>
                            <textarea id="body" name="body" style="display:none;"><?= $article['body'] ?></textarea>
                        </div>

                        <div class="form-group" style="margin-top: 24px;">
                            <label for="excerpt">Short Excerpt / Deck</label>
                            <textarea id="excerpt" name="excerpt" class="form-control" style="min-height: 100px;" placeholder="Brief summary (1-2 sentences)..."><?= htmlspecialchars($article['excerpt']) ?></textarea>
                        </div>
                    </div>
                    ...
                    <div class="admin-card">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                            <div class="form-group">
                                ...
                                <input type="text" name="seo_title" class="form-control" value="<?= htmlspecialchars($article['seo_title']) ?>" placeholder="Max 70 chars">
                            </div>
                            <div class="form-group">
                                <label>Focus Keyword</label>
                                <input type="text" name="seo_keyword" class="form-control" value="<?= htmlspecialchars($article['seo_keyword'] ?? '') ?>" placeholder="e.g. Punjab News">
                            </div>
                        </div>
                        <div class="form-group">
                            <label>Meta Description</label>
                            <textarea name="meta_desc" class="form-control" style="height: 60px;" placeholder="Search engine snippet..."><?= htmlspecialchars($article['meta_desc']) ?></textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="admin-card">
                        ...
                        <div id="image-preview" style="width: 100%; aspect-ratio: 16/9; background: rgba(255,255,255,0.02); border-radius: 12px; border: 2px dashed var(--admin-border); display: flex; align-items: center; justify-content: center; flex-direction: column; cursor: pointer; overflow: hidden; position: relative;" onclick="document.getElementById('image-upload').click()">
                            <?php if(!empty($article['image_path'])): ?>
                                <img id="preview-img" src="<?= SITE_URL . $article['image_path'] ?>" style="width: 100%; height: 100%; object-fit: cover;">
                            <?php else: ?>
                        ...
                        <input type="hidden" name="current_featured_image" value="<?= $article['featured_image'] ?>">
    ...
    function previewImage(input) {
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                const span = document.querySelector('#image-preview span');
